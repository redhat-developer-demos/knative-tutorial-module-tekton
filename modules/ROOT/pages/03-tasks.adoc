...
* **name** - the unique name using which the task will will be referred
...
[TIP]
====
* Use the command **help** via `tkn taskrun --help` or `tkn task --help`
...
You can check the created <<tekton-inspect-taskrun,TaskRun>> and the TaskRun usually takes time to complete, to check the status of the TaskRun use the `logs` command of taskrun like:
...
kubectl run demo-greeter -n {tutorial-namespace} \
 ...
 --image='localhost:5000/rhdevelopers/knative-tutorial-greeter' && \
...
[source,bash,subs="+macros,attributes+"]
----
oc run demo-greeter -n {tutorial-namespace} --generator='run-pod/v1' --image='localhost:5000/rhdevelopers/knative-tutorial-greeter' && \
