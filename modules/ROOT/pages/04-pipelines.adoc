...
IMPORTANT: Only resources of same type can be bound
...
[TIP]
====
* Use the command **help** via `tkn pipelinerun --help` or `tkn pipeline --help`
...
You can check the created <<pipelinerun,PipelineRun>> and the PipelineRun usually takes time to complete, to check the status of the PipelineRun use the `logs` command of pipelinerun like:
